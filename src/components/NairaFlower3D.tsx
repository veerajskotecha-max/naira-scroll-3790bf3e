...
/*
  The brand-deck flower, in the wordmark's blush and turning beside the header wordmark
  at the Naira box's pace.

  The header is on every page and above the fold, so three.js (~146 KB gzip)
  must not compete with the page for the first load. A flat blush still of the
...
const NairaFlower3D = ({
  ...
  return (
    <span aria-hidden="true" className={`pointer-events-none block aspect-[3/4] ${className}`} style={style}>
      <svg
        ...
        style={{ height: `${FILL * 100}%`, fill: "var(--nf-blush)" }}
      >
        <path d={PATH} />
      </svg>
      <canvas
