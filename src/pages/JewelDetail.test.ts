...
/*
  The PDP carried two pieces of invented social proof: a popup announcing that
  a randomly generated shopper from a random city had "just bought" a piece,
  and a "Last 24 hours: N pieces sold" line where N was a hash of the product
  handle — the same figure every day. Neither was true, and both were the
  clutter the owner asked to have removed. They must not creep back.
*/
describe("no fabricated social proof on the PDP", () => {
  it("does not mount the 'just bought' popup", () => {
    expect(code).not.toMatch(/<FomoPopup/);
  });

  it("does not show a made-up sold count in the trust strip", () => {
    const strip = readFileSync(resolve(__dirname, "../components/jewellery/JewelTrustStrip.tsx"), "utf8")
      .replace(/\/\*[\s\S]*?\*\//g, "");
    expect(strip).not.toMatch(/sold/i);
    expect(strip).not.toMatch(/hash/);
  });
});

/*
  Product information used to sit behind five closed accordion rows, so on a
  phone the description was never seen unless tapped open. It is now a
  horizontal tab strip whose first tab — details and description — is open on
  arrival.
*/
describe("product information opens on details and description", () => {
  it("defaults the tab strip to the details tab", () => {
    expect(code).toMatch(/<Tabs defaultValue="details"/);
    expect(code).toMatch(/\["details", "Details & Description"\]/);
  });

  it("renders the description inside the default tab", () => {
    const details = code.match(/<TabsContent value="details"[\s\S]*?<\/TabsContent>/)![0];
    expect(details).toMatch(/piece\.blurb/);
  });
});
